<template>
	<view>
		...
		<view v-show="current === 0">
			<regularWorkerDetail></regularWorkerDetail>
		</view>
		...
		<view v-show="current === 1">
			...
			<uni-list :border="true">
				<uni-list-item :border="true" uni_border_color="#a6a6a6" clickable v-for="(item,index) in leaveList"
					...
					:to="`/pages/business/regularWorker/regularWorkerDetail?type=view&id=${item.id}`">

					<template v-slot:body>
						<view class="slot-body">
							<view class="list_title ">
								<uni-title type="h2" title="转正申请"></uni-title>
							</view>
							<view class="list_body">
								<view class="list_body_content">申请人：{{item.applicant}}&nbsp;&nbsp;&nbsp;标题:{{item.title}}</view>
								<view class="list_body_content" v-if="item.createTime" style="color: #999999;">申请日期: {{item.createTime}}</view>
							</view>
						</view>
					</template>
		...
		<view class="uni-container">
			<uni-table ref="table" :loading="loading" border stripe type="selection" emptyText="暂无更多数据"
				...
				<uni-tr>
					...
					<uni-th align="center">入职日期</uni-th>
					<uni-th align="center">转正日期</uni-th>
				...
				<uni-tr v-for="(item, index) in leaveList" :key="index">
					...
					<uni-td align="center">{{ item.positiveDate }}</uni-td>
					<uni-td align="center">
...
<script>
	import regularWorkerDetail from './regularWorkerDetail'
	import {
		listInformation,
	...
	export default {
		...
		methods: {
			...
			// handleAdd() {
			// 	uni.navigateTo({
			// 		url: `pages/business/regularWorker/regularWorkerDetail?type=add`
			// 	});
			// },
			// // 修改事件
			// handleUpdate(row) {
			...
			// 	}
			// 	uni.navigateTo({
			// 		url: `pages/business/regularWorker/regularWorkerDetail?type=edit&id=${id}`
			// 	});
			// },
			// //撤回
			...
			// 		flag:row.actBusinessStatus.status==="waiting"?true:false
			// 	}
