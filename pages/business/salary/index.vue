		...
		<view v-show="current === 1">
			...
			<uni-list :border="true">
				<uni-list-item :border="true" uni_border_color="#a6a6a6" clickable v-for="(item,index) in leaveList"
					...
					:to="`/pages/business/salary/salaryDetails?type=view&id=${item.id}`">
					<template v-slot:body>
						<view class="slot-body">
							<view class="list_title ">
								<uni-title type="h3" :title="item.applicant"></uni-title>
								<DictTag :dict="act_statusList" :value="item.actBusinessStatus.status"></DictTag>
							</view>
							<view class="list_body">
								<view class="list_body_content">职务: {{item.office}}</view>
								<view class="list_body_content">毕业时间: {{item.graduationTime}}</view>
							</view>
						</view>	
					</template>
